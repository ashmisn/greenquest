...
import { Award, TrendingUp, Users, Leaf, Star, Gift, Recycle, Truck, Receipt, Smartphone, Ticket } from 'lucide-react';
import { Reward, User, Admin } from '../types'; // Import necessary types
...
const Dashboard: React.FC = () => {
  ...
  // useEffect to fetch rewards (for users only)
  useEffect(() => {
    if (!isAdmin && token) {
  ...
  // useEffect to fetch latest profile
  useEffect(() => {
  ...
  // Handler for Admin to update pickup status
  ...
  // Helper function to safely get user points
  const getUserPoints = () => {
      if (currentUser && currentUser.role === 'user') {
          return (currentUser as User).points || 0;
      }
      return 0;
  }
  
  // Helper to get an icon based on reward type
  const getRewardIcon = (type: Reward['type']) => {
    switch (type) {
        case 'Discount': return <Receipt className="w-5 h-5 text-blue-500 flex-shrink-0" />;
        case 'Recharge': return <Smartphone className="w-5 h-5 text-purple-500 flex-shrink-0" />;
        case 'Voucher': return <Ticket className="w-5 h-5 text-orange-500 flex-shrink-0" />;
        default: return <Gift className="w-5 h-5 text-gray-500 flex-shrink-0" />;
    }
  };
      ...
      <div className="max-w-6xl mx-auto px-4 py-12">
        {isAdmin ? (
          ...
          <div className="space-y-8">
             ...
             {/* --- Admin Content from your original file --- */}
          ...
          <div className="space-y-8">
            ...
            <div className="bg-gradient-to-r from-green-600 to-green-500 p-8 rounded-2xl text-white shadow-xl">
                <h1 className="text-3xl font-bold mb-4">Welcome back, {(currentUser as User).fullName}!</h1>
                 <div className="grid md:grid-cols-3 gap-6">
                   <div className="text-center">
                      <div className="text-3xl font-bold text-yellow-300">{(currentUser as User).points}</div>
                      <div className="text-green-100">Total Points</div>
                    </div>
                    <div className="text-center">
                      <div className="text-3xl font-bold text-yellow-300">{(currentUser as User).level}</div>
                      <div className="text-green-100">Current Level</div>
                    </div>
                    <div className="text-center">
                      <div className="text-3xl font-bold text-yellow-300">{(currentUser as User).village}</div>
                  ...
                  {loadingRewards ? (
                    ...
                    rewards.map(reward => (
                      <button 
                        ...
                        onClick={() => alert(`Redeeming ${reward.title}!`)}
                      >
                        <div className="flex justify-between items-start">
                           ...
                           <p className={`text-sm font-bold flex-shrink-0 ${getUserPoints() >= reward.pointsRequired ? 'text-green-600' : 'text-red-500'}`}>
